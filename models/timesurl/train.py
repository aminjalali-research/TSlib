...
if __name__ == '__main__':
    ...
    # Fix input_dims for specific datasets that have dimension mismatches
    calculated_input_dims = train_data['x'].shape[-1] - (1 if args.load_tp else 0)
    print(f"Calculated input_dims: {calculated_input_dims} (shape: {train_data['x'].shape}, load_tp: {args.load_tp})")
    
    # Dataset-specific overrides for input dimension mismatches
    if args.dataset == 'AtrialFibrillation' and calculated_input_dims > 2:
        # AtrialFibrillation should have 2 input dimensions (channels)
        # Slice the data to use only the first 2 channels
        print(f"Slicing data from {train_data['x'].shape} to use only first 2 channels for {args.dataset}")
        train_data['x'] = train_data['x'][..., :2]  # Keep only first 2 channels
        test_data['x'] = test_data['x'][..., :2]    # Keep only first 2 channels
        input_dims = 2
        print(f"New train_data shape: {train_data['x'].shape}")
    else:
        input_dims = calculated_input_dims
