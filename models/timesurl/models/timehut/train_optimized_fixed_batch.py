...
# Performance optimization flags - Fixed batch size for fair comparison
OPTIMIZATION_FLAGS = {
    ...
    'batch_size_scaling': False,    # DISABLED: Use fixed batch size for fair comparison
...
def get_amc_grid_ranges():
    ...
    return {
        'amc_instance': [0, 0.1, 0.5, 1, 3, 5],
        'amc_temporal': [0, 0.1, 0.5, 1, 3, 5],
        'amc_margin': [0.5]  # Fixed margin as in original scripts
    }
...
def create_amc_setting(amc_instance=0, amc_temporal=0, amc_margin=0.5):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--amc-instance', type=float, default=0, help='AMC coefficient for instance contrastive loss')
    parser.add_argument('--amc-temporal', type=float, default=0, help='AMC coefficient for temporal contrastive loss')
    ...
    if args.loader == 'UCR':
        ...
        data, train_slice, valid_slice, test_slice, scaler, pred_lens, n_covariate_cols = datautils.load_forecast_csv(args.dataset, dataroot=args.dataroot)
