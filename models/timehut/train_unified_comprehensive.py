...
def train_model_with_amc(train_data, train_labels, test_data, test_labels, device, config, temp_dictionary=None, amc_setting=None, epochs=200, iters=None, type="full"):
    ...
    if type == 'full':
        ...
        print(f"   📊 Epochs: {epochs}, Batch Size: {config.get('batch_size', 8)}")
        ...
        print(f"⏱️ Training completed in {time.time() - t:.2f}s")
...
if __name__ == '__main__':
    ...
    final_res = train_model_with_amc(
        train_data, train_labels, test_data, test_labels, device, 
        config, temp_dictionary=temp_settings, amc_setting=amc_setting, 
        epochs=args.epochs, iters=args.iters, type="full"
    )
